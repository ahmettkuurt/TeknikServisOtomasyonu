...
namespace TeknikServisOtomasyonu
{
    public partial class Frm_Persnel_AnaEkran : Form
    {
        ...
        void Plistele()
        {
            SqlCommand liste = new SqlCommand("Select AdiSoyadi,Telefon,Mail From Personeller Where P_Id=@p1", baglan.baglanti());
        ...
        void biten()
        {
            ...
            SqlDataAdapter da = new SqlDataAdapter("Select T.Teknik_Id, T.AdiSoyadi, T.Degisen_Parca, T.Yapilan_İslem, T.Teslim_Tarihi, P.AdiSoyadi, P.Telefon, " +
                ...
                "Where T.Ariza_Id = A.Ariza_Id And A.P_Id = P.P_Id And T.AdiSoyadi is not null And P.P_Id=@p1", baglan.baglanti());
        ...
        void listele() //DataGridView De Verileri Listeleme Fonksiyonu
        {
            ...
            SqlDataAdapter da = new SqlDataAdapter("Select A.Ariza_Id,P.AdiSoyadi,P.Telefon,P.Mail,A.Marka, A.Model, A.SeriNo, Ariza_Detay, " +
                ...
                " Where A.P_Id=P.P_Id And P.P_Id=@p1", baglan.baglanti());
            da.SelectCommand.Parameters.AddWithValue("@p1", Convert.ToInt16(id));
            da.Fill(dt);
        ...
        {
            SqlCommand komut = new SqlCommand("Select Ariza_Id From Ariza_Bilgisi Where Model=@p1 and SeriNo=@p2", baglan.baglanti());
            ...
            else
            {
                ...
                "values(@p1, @p2, @p3, @p4,@p5,@p6,@p7)", baglan.baglanti());//İnsert İşlemini Tanımladım. 
                ...
                baglan.baglanti().Close();//bağlantıyı kapat
        ...
        private void bttn_sil_Click(object sender, EventArgs e)
        {
                foreach (DataGridViewRow row in dataGridView1.SelectedRows)  //Seçili Satırları Silme
                {
                    ...
                    {
                        SqlCommand komut = new SqlCommand("Delete From Ariza_Bilgisi Where Ariza_Id = @p1", baglan.baglanti());
        ...
        private void bttn_guncelle_Click(object sender, EventArgs e)
        {
            SqlCommand komutguncelle = new SqlCommand("Update Ariza_Bilgisi Set Marka=@p4," +
                "Model=@p5,SeriNo=@p6,Ariza_Detay=@p7,Garanti=@p8,Gönderim_Tarihi=@p9 Where Ariza_Id=@p10", baglan.baglanti());
        ...
        private void bttn_son_Click(object sender, EventArgs e)
        {
            ...
            txtf_adi.Text = dataGridView1.Rows[i].Cells[0].Value.ToString();
            masked_telefon.Text = dataGridView1.Rows[i].Cells[1].Value.ToString();
            txtf_mail.Text = dataGridView1.Rows[i].Cells[2].Value.ToString();
            txtf_marka.Text = dataGridView1.Rows[i].Cells[3].Value.ToString();
            txtf_model.Text = dataGridView1.Rows[i].Cells[4].Value.ToString();
            masked_seri.Text = dataGridView1.Rows[i].Cells[5].Value.ToString();
            richTextBox1.Text = dataGridView1.Rows[i].Cells[6].Value.ToString();
            cmb_grnti.Text = dataGridView1.Rows[i].Cells[7].Value.ToString();
        ...
        private void bttn_ara_Click(object sender, EventArgs e)
        {
            ...
            "Model like '%" + txtf_ara.Text + "%' ", baglan.baglanti());
